...
<div class="container">
	<h3 class="mb-4">
		<i class="bi bi-bag-check me-2" style="color: #c9a84c;"></i>Mes
		Commandes
	</h3>
	...
	<c:if test="${empty orders}">
		<div class="text-center py-5">
			...
			<a href="${pageContext.request.contextPath}/products" class="btn"
				style="background: #3d1a00; color: #c9a84c;"> <i
				class="bi bi-grid me-2"></i>Parcourir le Catalogue
			</a>
		</div>
	</c:if>
					...
					<div class="card-body">
						<div
							...
							<div>
								...
								<p class="mb-0 small">
									<i class="bi bi-box-seam me-1"></i>${order.items.size()}
									article(s) &bull; <i class="bi bi-wallet2 me-1"></i>${order.paymentMethod}
								</p>
							</div>
							<div class="text-end">
								<div class="mb-2">
									<c:choose>
										...
										<c:when
											test="${order.status == 'CONFIRME' || order.status == 'EN_PREPARATION'}">
								...
								<a href="${pageContext.request.contextPath}/orders/${order.id}"
									...
									style="background: #3d1a00; color: #c9a84c;"> Détails <i
									class="bi bi-arrow-right ms-1"></i>
								</a> <a
									href="${pageContext.request.contextPath}/orders/${order.id}/receipt"
